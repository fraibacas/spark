...
minYear = onlyLabels.min() #takeOrdered(1)[0]
maxYear = onlyLabels.max() #takeOrdered(1, lambda x: -1*x)[0]
...
def calcRMSE(labelsAndPreds):
    ...
    """
    se_sum = labelsAndPreds.map(lambda x: squaredError(x[0], x[1])).reduce(lambda a, b: a+b)
    return math.sqrt(se_sum / labelsAndPreds.count())
...
def getLabeledPrediction(weights, observation):
    ...
    return ( label, weights.dot(features) )
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda x: gradientSummand(w, x)).reduce(lambda a,b: a + b)
...
def twoWayInteractions(lp):
    ...
    """
    new_features = lp.features.tolist()
    for x, y in itertools.product(lp.features, lp.features):
        #combined = [math.pow(x, 2), x*y, y*x, math.pow(y, 2)]
