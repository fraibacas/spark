...
def simpleTokenize(string):
    ...
    """
    return [ w.strip().lower() for w in re.split(split_regex, string) if w and w.strip() ]
...
def tf(tokens):
    ...
    tokens_dict = {}
    for tk in tokens:
        if tk in tokens_dict.keys():
            tokens_dict[tk] = tokens_dict[tk] + 1
        else:
            tokens_dict[tk] = 1

    n_tokens = len(tokens)
    for tk in tokens_dict.keys():
        tokens_dict[tk] =  float(tokens_dict[tk]) / float(n_tokens)
    return tokens_dict
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda x: set(x[1])).distinct()
    tokenCountPairTuple = uniqueTokens.cartesian(corpus).map(lambda x: (x[0], 1 if x[0] in x[1][1] else 0))
...
similarities = (crossSmall
                ...
                .filter(lambda x: x[2]>=0 and x[2]<=1)
                .cache())
...
trueDupsRDD = (sims
               .map(lambda x: ('{0} {1}'.format(x[0],x[1]), x[2]))
               .join(goldStandard)
              )
...
avgSimNon = nonDupsRDD.map(lambda x: x[1][0]).reduce(lambda a, b: a+b) / float(nonDupsRDD.count())
...
amazonInvPairsRDD = (amazonWeightsRDD
                    .flatMap(lambda x: invert( (x[0], x[1].keys()) ))
                    .cache())
...
def fastCosineSimilarity(record):
    ...
    s = 0
    for tk in tokens:
        s = s + amazonWeightsBroadcast.value[amazonRec][tk] * googleWeightsBroadcast.value[googleRec][tk]
