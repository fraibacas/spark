...
hosts = access_logs.map(lambda x: (x.host, 1)).reduceByKey(lambda x, y: x + y)

uniqueHosts = hosts.keys()
...
badEndpoints = badRecords.map(lambda log: (log.endpoint, 1)).reduceByKey(lambda e1, e2: e1+e2)
...
badEndpointsSum = badEndpointsCountPairTuple.reduceByKey(lambda e1, e2: e1+e2)
...
errHostsSum = errHostsCountPairTuple.reduceByKey(lambda e1, e2: e1+e2)
...
errDateSum = errDateCountPairTuple.reduceByKey(lambda e1, e2: e1+e2)
...
hourRecordsSum = hourCountPairTuple.reduceByKey(lambda e1, e2: e1+e2)
