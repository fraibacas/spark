...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda r: ((r[0], r[1]), r[2]))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda r: ((r[0], r[1]), r[2]))

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                        .map(lambda x: math.pow(x[1][0]-x[1][1], 2))
                       )
...
myRatedMovies = [ (0, random.randint(1, 1500), random.uniform(0.0, 5.0)) for x in range(15) ]
...
# TODO: Replace <FILL IN> with appropriate code
predictedTestMyRatingsRDD = testForPredictingRDD
testRMSEMyRatings = myRatingsModel.predictAll(predictedTestMyRatingsRDD)

print 'The model had a RMSE on the test set of %s' % computeError(testRDD, testRMSEMyRatings)
...
# TODO: Replace <FILL IN> with appropriate code
myRatedMovies_ids = { movie for (user, movie, rating) in myRatedMovies }
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda x: x[1] not in myRatedMovies_ids)
                      .map(lambda x: (0, x[1])))
...
#print movieIDsWithAvgRatingsRDD.take(1)
